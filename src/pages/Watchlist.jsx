...
// Detect current US market session
function getMarketSession() {
  ...
  // EST = UTC-5 (or UTC-4 during EDT, close enough)
  const estMin = ((totalMin - 5 * 60) + 1440) % 1440;

  if (dayOfWeek === 0 || dayOfWeek === 6) return 'closed';
  if (estMin >= 240 && estMin < 570)  return 'pre';   // 4:00am–9:30am EST
  if (estMin >= 570 && estMin < 960)  return 'open';  // 9:30am–4:00pm EST
  if (estMin >= 960 && estMin < 1200) return 'after'; // 4:00pm–8:00pm EST
  return 'closed';
}
...
function MarketSessionBadge() {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border ${cfg.cls}`}>
      {session !== 'open' && <span className="w-1.5 h-1.5 rounded-full bg-current mr-1 opacity-70" />}
      {cfg.label}
    </span>
  );
}
    ...
    <div className="space-y-5 pb-20 md:pb-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/30">Track your favorite assets • Live prices • Long-press to reorder</p>
        </div>
